#version 150
// Black-Hot Thermal Vision Shader
// Monochrome sibling of thermal-vision.fsh - identical gain/blur/noise pipeline so
// all three palettes behave like the same device, only the output mapping differs:
// cold = white, hot = black. This is the polarity most imagers default to for
// spotting warm bodies against cool terrain, since a dark target on a pale field
// reads faster than the reverse.
...
// Minimum thermal signal, so the coldest thing on screen still carries some value
// rather than being a void. Same meaning as in the white-hot version - this is the
// signal floor, before the display polarity is applied.
const float COLD_FLOOR = 0.03;

// ...and the matching display floor once the signal is inverted, so the hottest
// target reads as very dark rather than crushing to pure black.
const float HOT_FLOOR = 0.03;
...
vec4 blackHotVision(sampler2D tex, vec2 uv, vec2 fragCoord) {
    ...
    // Everything above this point is the same thermal signal the white-hot version builds; only the
    // display mapping differs, with the signal inverted so a hot body comes out dark on a pale field.
    //
    // The vignette has to be split in two to do that. It peaks at 1.728 in the middle of the screen, not
    // at 1.0, so it is really a centre gain as much as an edge falloff. White-hot multiplies its signal by
    // the whole thing and lets the clamp absorb the overshoot. Doing the same after inverting blew the
    // cold background - which inverts to nearly 1.0 - straight through the clamp, painting a white ellipse
    // over the middle of the frame wherever the vignette exceeded 1.0. At night, when the whole scene is
    // cold, that covered most of the view.
    //
    // So: the gain stays on the signal, exactly as white-hot applies it, and only the normalised falloff
    // is applied to the inverted value, where it can darken the surround but never brighten the centre.
    float vignettePeak = (VignetteEnabled > 0.5) ? 1.728 : 1.0;   // pow(1.2, 3.0)
    float surround = clamp(vignette / vignettePeak, 0.0, 1.0);

    float signal = clamp((heat + thermalNoise) * vignette, 0.0, 1.0);
    float value = max(1.0 - signal, HOT_FLOOR) * surround;

    return vec4(vec3(value), 1.0);
}

void main() {
    fragColor = blackHotVision(DiffuseSampler, texCoord, texCoord * InSize);
}
